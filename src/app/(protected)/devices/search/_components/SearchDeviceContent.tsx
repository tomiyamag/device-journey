...
import { InputClassNames } from "@/components/ui/FormInput";
import FormLabel from "@/components/ui/FormLabel";

import { useDeviceSearchStore } from "../../_stores/useDeviceSearchStore";
import { useAutocompleteMobileDevices } from "../_hooks/useMobileApi";
import { AutocompleteDeviceMobileApiResult } from "../_types";
import SearchStatusMessage from "./SearchStatusMessage";

export const minLength = 1;

const SearchDeviceContent = () => {
      ...
      >
        <div className="relative">
          <ComboboxInput
            ...
            displayValue={(device: AutocompleteDeviceMobileApiResult) =>
              device?.name || query
            }
            onChange={(event) => {
        ...
        {isSuggestionsShow && (
          ...
          >
            {isFetching && (
              <SearchStatusMessage label="デバイスを検索しています..." />
            )}

            {isError && (
              <SearchStatusMessage label="通信エラーが発生しました" />
            )}

            {!isFetching && !isError && !hasData && (
              <SearchStatusMessage label="デバイスが見つかりませんでした" />
            )}

            {!isFetching && hasData && (
              <>
                {data.map((device: AutocompleteDeviceMobileApiResult) => (
                  <ComboboxOption
                    key={device.id}
                    value={device}
...
export default SearchDeviceContent;
